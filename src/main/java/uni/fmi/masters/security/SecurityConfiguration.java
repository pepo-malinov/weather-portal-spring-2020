...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	public SecurityConfiguration(final UserDetailsService userDetailsService, final PasswordEncoder passwordEncoder) {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().requestCache().requestCache(new CustomRequestCache()).and().authorizeRequests()
				.requestMatchers(request -> SecurityUtils.isFrameworkInternalRequest(request)).permitAll().anyRequest()
				.authenticated().and().formLogin().loginPage(LOGIN_URL).permitAll()
